...
function candidate(over: Partial<DealCandidate> = {}): DealCandidate {
  return {
    ...
    sourceUrl: 'https://pub.test/specials',
...
describe('confidenceScore', () => {
  ...
  it('llm path scores below an equivalent structured one', () => {
    expect(confidenceScore(candidate({ extractionPath: 'llm' }))).toBeLessThan(
      confidenceScore(candidate()),
    );
  });
  it('missing fields lower the score', () => {
    expect(confidenceScore(candidate({ merchant: '', address: '' }))).toBeLessThan(
      confidenceScore(candidate()),
    );
  });
  it('clamps to 0–100', () => {

...
describe('FirecrawlBudgetService', () => {
  it('allows when usage is under caps', async () => {
    const svc = new FirecrawlBudgetService(fakePrisma({ totalPages: 10, sourcePages: 2, recrawls: 0 }) as never, limits);
    expect((await svc.check('s1', { sourceMayBeUnchanged: false })).allowed).toBe(true);
  });

  it('blocks once the daily page cap is hit', async () => {
    const svc = new FirecrawlBudgetService(fakePrisma({ totalPages: 100, sourcePages: 2, recrawls: 0 }) as never, limits);
  ...
  it('blocks a maybe-unchanged source that hit the recrawl cap', async () => {
    const svc = new FirecrawlBudgetService(fakePrisma({ totalPages: 10, sourcePages: 2, recrawls: 2 }) as never, limits);
